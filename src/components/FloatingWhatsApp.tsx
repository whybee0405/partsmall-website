 ...
 *
 * `branches` comes from the root layout, which fetches the live list
 * server-side — this component can't query Payload itself (client-side,
 * and it has no route params to know which branch page it's on anyway).
 */
export function FloatingWhatsApp({ branches }: { branches: Branch[] }) {
  ...
  const branch = branchSlug ? branches.find((b) => b.slug === branchSlug) : undefined
